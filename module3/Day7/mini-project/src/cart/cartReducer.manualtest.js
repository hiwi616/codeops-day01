// Call cartReducer directly with plain objects, no React, no test runner.
// Run with: node src/cart/cartReducer.manualtest.js
...
// unknown action: throws, instead of silently doing nothing
try {
  cartReducer(initialCartState, { type: "not-a-real-action" });
  console.log("FAIL - unknown action should throw");
} catch (err) {
  const pass = err.message === "Unknown action: not-a-real-action";
  console.log(`${pass ? "PASS" : "FAIL"} - unknown action throws a clear message`);
  if (!pass) console.log("  actual message:", err.message);
}
